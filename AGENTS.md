...
- The current provisional CAM fact boundary owns CAM root/version/top-level fields,
  ...
  static policy diagnostics. It does not own route execution, ABI compatibility,
  UI handoff/dataflow/typeflow, resource byte loading or integrity validation,
  wallet/RPC/account state, or runtime vs. conformance reporting policy. Do not
  add broader route facts until the smaller substrates prove insufficient.
