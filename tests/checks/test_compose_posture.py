...
class ComposePostureTest(unittest.TestCase):
    ...
    def test_node_package_workspace_stager_is_image_owned(self) -> None:
        ...
        self.assertIn('cp -a "$source_dir/package.json"', stager_text)
        self.assertIn('for manifest in "$source_dir"/*/package.json', stager_text)
        self.assertIn('rm -rf "$target_dir/$package_name/dist"', stager_text)
