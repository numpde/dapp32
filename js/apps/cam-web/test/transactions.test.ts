...
test("receipt timeout survives hostile follow-up diagnosis errors", async () => {
  const hostile = {
    toString() {
  ...
  await assert.rejects(
    ...
    /The viewer also could not inspect the submitted transaction on localhost:8545: unprintable error\./,
  )
})
