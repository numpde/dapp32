import { isRecordObject, joinPath } from "./json.ts"
...
export class InertValueError extends Error {
...
function inertError(message: string, path: string): InertValueError {
  // The root value has no public path label. Nested failures still include
  // their constructed path, such as owner.address.
  return new InertValueError(message, path === "" ? undefined : path)
}
